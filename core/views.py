...
def dashboard(request):
    ...
    for course in courses:
        level_num = course.semester.level.level
        semester_name = course.semester.name

        if level_num not in levels_data:
            levels_data[level_num] = {
                'first': {'courses': [], 'units': 0, 'points': 0},
                'second': {'courses': [], 'units': 0, 'points': 0}
            }

        sem_key = 'first' if semester_name == 'First' else 'second'
        levels_data[level_num][sem_key]['courses'].append(course)
        levels_data[level_num][sem_key]['units'] += course.unit
        levels_data[level_num][sem_key]['points'] += course.unit * grade_map.get(course.grade, 0)
    ...
    for level_num in levels_data:
        for sem in ['first', 'second']:
            sem_data = levels_data[level_num][sem]
            sem_data['gpa'] = sem_data['points'] / sem_data['units'] if sem_data['units'] > 0 else 0
